...
ofMesh buildTerrainMesh(const Heightfield & H, const TerrainWorld & W) {
	...
	for (int yy = 0; yy < h; ++yy) {
		for (int xx = 0; xx < w; ++xx) {
			...
			const float Yh = hN * W.heightScale;
	...
	for (int yy = 0; yy < h; ++yy) {
		for (int xx = 0; xx < w; ++xx) {
			...
			if (hN < 0.5f) {
				...
				const ofFloatColor low(0.22f, 0.42f, 0.28f, 1.0f);
...
void colorByLogDrainage(ofMesh & mesh, const Heightfield & H, const Heightfield & drainageField) {
	...
	for (int yy = 0; yy < h; ++yy) {
		for (int xx = 0; xx < w; ++xx) {
			...
			const ofFloatColor c0(0.05f, 0.10f, 0.30f, 1.0f);
			const ofFloatColor c1(0.10f, 0.70f, 0.80f, 1.0f);
			const ofFloatColor c2(0.95f, 0.97f, 1.00f, 1.0f);
